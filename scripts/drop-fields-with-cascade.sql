-- SQL для безопасного удаления дублирующихся полей с пересозданием views
-- Выполните этот скрипт в Supabase SQL Editor

-- 1. Сначала удаляем views, которые зависят от старых полей
DROP VIEW IF EXISTS ton_farming_data CASCADE;
DROP VIEW IF EXISTS uni_farming_data CASCADE;
DROP VIEW IF EXISTS referrals CASCADE;
DROP VIEW IF EXISTS farming_status_view CASCADE;

-- 2. Теперь можем безопасно удалить колонки
...
-- 3. Пересоздаем views с использованием основных полей

-- View для UNI farming (использует основные поля)
...
SELECT 
  ...
  uni_deposit_amount as farming_deposit,  -- Теперь используем основное поле
...
-- View для TON farming (использует основные поля)
...
SELECT 
  ...
  ton_boost_package as boost_package_id,  -- Теперь используем основное поле
...
-- View для referrals (без изменений - не использует удаляемые поля)
...
-- Должно вернуть 4 строки с названиями views