-- БЕЗОПАСНОЕ УДАЛЕНИЕ ДУБЛИРУЮЩИХСЯ ПОЛЕЙ
-- Используем CASCADE для автоматического удаления зависимых объектов

-- 1. Удаляем колонки с CASCADE - это автоматически удалит все зависимые views
...
-- 2. Проверяем, что колонки удалены
SELECT column_name 
...
-- 3. Теперь пересоздаем views с правильными полями
...
SELECT 
  ...
  uni_deposit_amount as farming_deposit,  -- Используем основное поле
...
SELECT 
  ...
  ton_boost_package as boost_package_id,  -- Используем основное поле
