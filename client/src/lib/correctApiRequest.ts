...
export async function correctApiRequest<T = any>(
  ...
  // Добавляем guest_id если доступен
  try {
    // Импортируем sessionStorageService и извлекаем guest_id
    const { getGuestId } = await import('@/services/sessionStorageService');
    const guestId = getGuestId();
    if (guestId) {
      headers['X-Guest-Id'] = guestId;
    }
  } catch (error) {
    console.warn('[correctApiRequest] Не удалось получить guest_id из localStorage:', error);
  }
