...
export async function sendTonTransaction(
  ...
  try {
    ...
    // По новому ТЗ: создаем упрощенную версию BOC-payload
    // В данном случае мы вернемся к простой base64 кодировке, 
    // так как у нас возникают проблемы с Buffer в браузерном окружении
    
    // Вместо использования @ton/core для создания полноценного BOC
    // для целей тестирования и дебага используем обычный base64 encoded текст
    const payload = btoa(rawPayload);
    
    // Логируем, что используем упрощенный вариант
    console.log("⚠️ Используем упрощенный вариант payload (только base64) из-за проблем с Buffer");
