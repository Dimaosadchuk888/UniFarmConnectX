...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  // Отложенная инициализация TonConnect для предотвращения React Hook ошибок
  useEffect(() => {
    const initTonConnect = () => {
      console.log('[UserContext] Начинаем отложенную инициализацию TonConnect...');
      ...
      setTimeout(() => {
        try {
          ...
          if (window.Telegram?.WebApp?.ready) {
            ...
            console.log('[UserContext] ✅ TonConnect успешно инициализирован отложенно');
          } else {
            console.log('[UserContext] Telegram WebApp не готов, продолжаем без TonConnect');
            setIsTonConnectReady(true); // Продолжаем работу без TonConnect
          }
        } catch (error) {
