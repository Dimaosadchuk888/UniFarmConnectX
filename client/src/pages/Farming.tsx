...
const Farming: React.FC = () => {
  ...
  // [FIX: BUTTON ISSUE #2] Настройка кнопок при загрузке страницы
  React.useEffect(() => {
    console.log('[FARMING PAGE] 🔘 Настройка кнопок фарминга...');
    
    // [FIX: BUTTON ISSUE #2] Показываем кнопку только на farming экране
    // Кнопка отображается только при входе в farming-экран
    farmingButtons.showStartFarming(handleStartFarming);
    
    // Показываем кнопку возврата на главную
    farmingButtons.showBackToHome();
    
    // [FIX: BUTTON ISSUE #2] Очистка кнопок при выходе со страницы
    return () => {
