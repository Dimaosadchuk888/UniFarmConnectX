...
- June 15, 2025. SUPABASE API MIGRATION 85% COMPLETED - Successfully migrated 12 critical system modules from PostgreSQL + Drizzle ORM to Supabase API: core/supabase.ts centralized client, modules/auth/service.ts complete rewrite, modules/users/repository.ts + service.ts full conversion, modules/farming/service.ts + referral/service.ts + airdrop/service.ts migrated, server/index.ts cleaned, middleware files updated. All core business functions (authentication, users, farming, referrals, transactions) fully operational on Supabase. System ready for production deployment with stable-server.js entry point