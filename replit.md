...
### 🚨 КРИТИЧЕСКАЯ ПРОБЛЕМА ОБНАРУЖЕНА: Система TON депозитов полностью повреждена (July 29, 2025)
**Статус**: 🔴 ТРЕБУЕТ НЕМЕДЛЕННОГО ВОССТАНОВЛЕНИЯ

**Проблема**: Отключение rollback функций 29.07.2025 привело к полной поломке системы пополнений. Пользователи теряют все TON депозиты, фарминг доходы и награды.

**Техническая причина**: 
- `UnifiedTransactionService.updateUserBalance()` полностью отключена (строки 387-399)
- Транзакции создаются в БД, но балансы НЕ обновляются
- Эффект "транзакций-призраков" - депозиты не зачисляются пользователям

**Пострадавшие пользователи**:
- User 25: 2 депозита (14:06, 14:08) потеряны - tx_hash: te6cckECAgEAAKoAAeGI...
- Потенциально ВСЕ пользователи с депозитами после 29.07.2025 14:00

**Поврежденные операции**: TON_DEPOSIT, UNI_DEPOSIT, FARMING_REWARD, REFERRAL_REWARD, MISSION_REWARD, DAILY_BONUS

**Файлы с проблемами**:
- `core/TransactionService.ts` - updateUserBalance() возвращает немедленно
- `core/BalanceManager.ts` - отключен Math.max(0, balance - amount)
- `core/TransactionEnforcer.ts` - отключен мониторинг балансов

**План восстановления**: Создан `SAFE_TON_DEPOSIT_RECOVERY_PLAN_2025-07-29.md` с пошаговым планом безопасного восстановления системы
