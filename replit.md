...
- June 14, 2025. COMPLETE SUPABASE API TRANSITION - Полностью перешли с PostgreSQL на Supabase API по требованию пользователя. Установлен @supabase/supabase-js, создан core/supabaseClient.ts, обновлены 7 ключевых модулей: AuthService, UserRepository, WalletService, FarmingScheduler, UserRepository, AirdropService для работы с supabase.from().select/insert/update операциями. Удален drizzle.config.ts, создан SQL скрипт create-supabase-schema.sql для настройки базы данных в Supabase. Создан подробный отчет SUPABASE_MIGRATION_REPORT.md. Система готова к работе через Supabase API вместо прямого PostgreSQL подключения. Все следы Neon и PostgreSQL полностью исключены из проекта.