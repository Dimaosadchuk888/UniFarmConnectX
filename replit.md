...
1. **withdrawalService.ts**: Добавлена правильная классификация ошибок по типам (auth, validation, server, network)
2. **WithdrawalError interface**: Добавлено поле `error_type` для различения типов ошибок
3. **WithdrawalForm.tsx**: Улучшена обработка различных типов ошибок с соответствующими сообщениями
4. **Результат**: Auth failures теперь показывают "Требуется повторная авторизация" вместо "Нет подключения к интернету"
