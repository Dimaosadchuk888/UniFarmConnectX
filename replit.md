...
**Статус**: ✅ **ИСПРАВЛЕНО** - точная техническая причина найдена и полностью устранена.

**Исправления применены**:
1. **withdrawalService.ts**: Добавлена правильная классификация ошибок по типам (auth, validation, server, network, business_logic)
2. **WithdrawalError interface**: Добавлен тип `business_logic_error` для business logic ошибок
3. **WithdrawalForm.tsx**: Улучшена обработка различных типов ошибок с соответствующими сообщениями
4. **Business Logic Errors**: Добавлена обработка API ошибок типа "Недостаточно средств" - теперь показывается точное сообщение от сервера
5. **Результат**: 
   - Auth failures: "Требуется повторная авторизация" 
   - Business errors: точное сообщение от API (например, "Недостаточно средств. Доступно: X TON")
   - Network errors: "Проблемы с сетью" только для реальных network failures
