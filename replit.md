...
### 🔧 ТЕКУЩАЯ ПРОБЛЕМА: Админ бот не отвечает на команды (July 30, 2025)
**Статус**: В процессе диагностики

**Проблема**: Админ бот @unifarm_admin_bot не реагирует на команды /start и другие команды в Telegram.

**Выявленные причины**:
1. Webhook возвращает 404 Not Found при запросах от Telegram
2. Администраторы успешно добавлены в БД с флагом is_admin=true
3. ADMIN_BOT_TOKEN настроен правильно (46 символов)
4. Код интеграции готов - `adminBotService.notifyWithdrawal()` вызывается при создании заявок
5. Сервер периодически падает с ошибкой 502 Bad Gateway

**Ожидаемый результат после исправления**:
- Админы будут получать уведомления о новых заявках на вывод
- Бот будет отвечать на команды администраторов
- В уведомлениях будут кнопки "Одобрить" и "Отклонить"
