...
### 🚨 КРИТИЧЕСКАЯ ПРОБЛЕМА: Циклическое исчезновение балансов (July 29, 2025)
**Статус**: 🔴 ТРЕБУЕТ ВОССТАНОВЛЕНИЯ СИНХРОНИЗАЦИИ

**Проблема**: Балансы пользователей циклически появляются и исчезают после депозитов из-за отключенных функций синхронизации.

**Техническая причина**: 
- `UnifiedTransactionService.updateUserBalance()` ОТКЛЮЧЕНА для защиты от rollback (строки 387-399)
- Транзакции создаются в БД, но балансы НЕ обновляются автоматически
- Конфликт между 3 системами: WebSocket (real-time), API кеш (10 сек), автообновление (30 сек)
- Результат: "транзакции-призраки" - депозиты есть в БД, но не отражаются на балансе

**Архитектурный анализ**:
- Создан детальный отчет: `BALANCE_ARCHITECTURE_DIAGNOSTIC_REPORT_2025-07-29.md`
- Анализ влияния восстановления: `BALANCE_SYNC_RESTORATION_IMPACT_ANALYSIS_2025-07-29.md`

**План решения (ВАРИАНТ 1 - рекомендован)**:
1. Восстановить `updateUserBalance()` с дополнительной защитой от откатов
2. Добавить проверки на подозрительные операции (rate limiting, большие суммы)
3. Усилить логирование всех изменений балансов
4. Поэтапное внедрение: сначала TON_DEPOSIT → затем другие типы
5. Мониторинг 24-48 часов с готовностью к откату

**Критические функции для восстановления**:
- `UnifiedTransactionService.updateUserBalance()` - включить с защитой
- `BalanceManager.subtractBalance()` - разблокировать для withdrawals
- Оставить `recalculateUserBalance()` отключенным (опасно)

**Метрики мониторинга**: обновления/мин, расхождения транзакций, заблокированные операции, задержки
