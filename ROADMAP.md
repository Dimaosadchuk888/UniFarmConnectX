...
PUT    /:id             - Обновление пользователя по ID ✅ ДОБАВЛЕН
POST   /ref-code        - Генерация реферального кода ✅ ДОБАВЛЕН
POST   /recover-ref-code - Восстановление реферального кода ✅ ДОБАВЛЕН
```
...
GET    /                - Получение данных кошелька ✅ ДОБАВЛЕН
GET    /data            - Альтернативный endpoint для данных ✅ ДОБАВЛЕН
GET    /:userId/transactions - Транзакции пользователя по ID ✅ ДОБАВЛЕН
POST   /deposit-internal - Внутренний депозит ✅ ДОБАВЛЕН
POST   /withdraw-internal - Внутренний вывод ✅ ДОБАВЛЕН
```
...
POST   /deposit         - Депозит для фарминга ✅ ДОБАВЛЕН
GET    /balance         - Баланс фарминга ✅ ДОБАВЛЕН
```
...
GET    /farming-status  - Статус TON фарминга ✅ ДОБАВЛЕН
GET    /user-packages   - Пакеты пользователя ✅ ДОБАВЛЕН
GET    /stats           - Статистика boost системы ✅ ДОБАВЛЕН
```
...
GET    /user-missions   - Миссии конкретного пользователя ✅ ДОБАВЛЕН
GET    /available       - Доступные миссии ✅ ДОБАВЛЕН
```
...
GET    /monitor/status  - Статус критических endpoints ✅ ДОБАВЛЕН
GET    /monitor/logs    - Системные логи ✅ ДОБАВЛЕН
GET    /admin/test      - Тестовый админ endpoint ✅ ДОБАВЛЕН
POST   /admin/missions/manage - Управление миссиями ✅ ДОБАВЛЕН
```
...
GET    /webhook         - Статус webhook ✅ ДОБАВЛЕН
POST   /send-message    - Отправка сообщений ✅ ДОБАВЛЕН
```

### Admin Bot System (/api/v2/admin-bot)
```
POST   /webhook         - Webhook для админ-бота ✅ ДОБАВЛЕН
```
