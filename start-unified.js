...
/**
 * Загрузка переменных окружения с ПРИНУДИТЕЛЬНЫМ приоритетом для Neon DB
 */
function loadEnvironment() {
  log(`\n${colors.blue}=== Загрузка переменных окружения ===${colors.reset}`);
  log(`🚀 НАСТРОЙКА ПРИНУДИТЕЛЬНОГО ИСПОЛЬЗОВАНИЯ NEON DB`, colors.magenta);
  ...
  if (fs.existsSync(neonEnvPath)) {
    ...
    log(`⚠️ Файл .env.neon не найден, пытаемся получить DATABASE_URL из окружения`, colors.yellow);
    
    // Если нет .env.neon, загружаем стандартный .env
  ...
  log(`✅ ПРИНУДИТЕЛЬНО установлены настройки для использования Neon DB`, colors.green);
...
async function main() {
  ...
  log(`${colors.magenta}= ЗАПУСК UNIFARM С NEON DATABASE =${colors.reset}`);
  log(`${colors.magenta}===================================${colors.reset}`);
  
  // Загрузка переменных окружения с приоритетом Neon DB
  loadEnvironment();
  
  // Проверяем DATABASE_URL - это критически важно для Neon DB
  if (!process.env.DATABASE_URL || !process.env.DATABASE_URL.includes('neon.tech')) {
    log(`⚠️ ВНИМАНИЕ: DATABASE_URL отсутствует или не ведет на Neon DB!`, colors.yellow);
    log(`⚠️ Проверьте конфигурацию в .env или .env.neon файле`, colors.yellow);
    log(`⚠️ DATABASE_URL должен содержать подстроку 'neon.tech'`, colors.yellow);
    log(`⚠️ Продолжаем запуск, но возможны проблемы с подключением к БД...`, colors.yellow);
  }
  
  // Всегда используем Neon DB
  log(`ℹ️ Используется Neon Database (${process.env.DATABASE_PROVIDER})`, colors.blue);
