...
function validateTelegramInitData(initData: string, botToken: string): boolean {
  try {
    ...
    console.error('[StableTelegramMiddleware] Ошибка валидации initData:', error);
    return false;
  }
}
...
async function createOrUpdateTelegramUser(userData: any, startParam?: string): Promise<any> {
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      console.log('[StableTelegramMiddleware] Пользователь обновлен:', {
      ...
      console.log('[StableTelegramMiddleware] Новый пользователь создан:', {
        telegram_id: telegramId,
        username,
        ref_code: refCode,
        ref_by: refBy,
    ...
    console.error('[StableTelegramMiddleware] Ошибка работы с базой данных:', error);
    throw error;
  }
}
 ...
 */
export const stableTelegramMiddleware = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!initData && req.url.includes('tgWebAppData=')) {
      try {
        ...
        console.log('[StableTelegramMiddleware] Не удалось извлечь initData из URL');
      }
    }

    if (!initData) {
      console.log('[StableTelegramMiddleware] initData не найден в запросе');
      req.telegram = { validated: false };
      return next();
    }

    console.log('[StableTelegramMiddleware] Обработка initData, длина:', 
    ...
    if (botToken && process.env.VALIDATE_TELEGRAM_DATA !== 'false') {
      ...
      console.log('[StableTelegramMiddleware] Результат валидации initData:', isValid);
    } else {
      console.log('[StableTelegramMiddleware] Валидация пропущена (development режим)');
    }
    ...
    if (userParam) {
      ...
      console.log('[StableTelegramMiddleware] Данные пользователя извлечены:', {
        id: userData.id,
      ...
      try {
        ...
        console.log('[StableTelegramMiddleware] Пользователь успешно обработан и готов к использованию:', {
          id: telegramUser.id,
        ...
        console.error('[StableTelegramMiddleware] Ошибка работы с базой данных:', dbError);
        req.telegram = { validated: false };
      }

    } else {
      console.log('[StableTelegramMiddleware] Параметр user не найден в initData');
      req.telegram = { validated: false };
    }

  } catch (error) {
    console.error('[StableTelegramMiddleware] Критическая ошибка обработки:', error);
    req.telegram = { validated: false };
  }
