...
export async function isRefCodeUnique(refCode: string): Promise<boolean> {
  try {
    ...
    console.error('[RefCodeUtils] Ошибка при проверке уникальности кода:', error);
    throw error;
  }
}
...
export async function generateUniqueRefCode(): Promise<string> {
  ...
  console.log(`[RefCodeUtils] Сгенерирован уникальный код ${refCode} за ${attempts} попыток`);
  return refCode;
}
...
/**
 * Очищает реферальный код от возможных префиксов
 */
export function cleanRefCode(refCode: string): string {
  if (!refCode) return '';
  
  // Убираем префикс "ref_" если он есть
  if (refCode.startsWith('ref_')) {
    return refCode.substring(4);
  }
