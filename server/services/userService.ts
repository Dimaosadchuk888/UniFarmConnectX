  ...
  return {
    ...
    async getUserByGuestId(guestId: string): Promise<User | undefined> {
      ...
      try {
        ...
        // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: всегда возвращаем undefined вместо выброса ошибки
        // Это предотвращает 500 ошибки и обеспечивает стабильную работу API
        console.warn(`[UserService] 🔄 Возвращаем undefined для стабильности API, ошибка логируется`);
        return undefined;
      }
    },
