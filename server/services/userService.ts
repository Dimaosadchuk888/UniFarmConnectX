  ...
  return {
    ...
    async getUserByGuestId(guestId: string): Promise<User | undefined> {
      ...
      try {
        ...
        // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: НИКОГДА не выбрасываем ошибки из этого метода
        // ВСЕ ошибки обрабатываем как "пользователь не найден"
        console.warn(`[UserService] 🔄 Все ошибки трактуются как "пользователь не найден" для стабильности API`);
        return undefined;
      }
    },
