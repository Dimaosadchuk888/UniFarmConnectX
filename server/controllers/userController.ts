...
export const UserController = {
  ...
  async getUserByGuestId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // ИСПРАВЛЕНИЕ: Убираем вложенный try-catch и упрощаем логику
      ...
      // ВСЕ ошибки трактуем как "пользователь не найден" для стабильности API
      logger.error(`[UserController] ❌ Ошибка при получении пользователя по guest_id: ${guestId}`, error);
      logger.warn(`[UserController] 🔄 Возвращаем 404 вместо 500 для стабильности API`);
      sendError(res, 'Пользователь с указанным guest_id не найден', 404);
    }
  },
