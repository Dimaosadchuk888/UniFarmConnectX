...
export const UserController = {
  ...
  /**
   * Создает нового пользователя через Telegram регистрацию
   * [TG REGISTRATION FIX] Метод для корректной регистрации через telegram_id
   */
  async createUserFromTelegram(req: Request, res: Response): Promise<void> {
    try {
      logger.info('[UserController] [TG REGISTRATION] Получен запрос на регистрацию Telegram пользователя');
      
      const { telegram_id, username, first_name, last_name, language_code, parent_ref_code } = req.body;
      
      // Валидация входных данных
      if (!telegram_id) {
        return sendError(res, 'telegram_id обязателен для Telegram регистрации', 400);
      }
      
      // Проверяем, не существует ли уже пользователь с таким telegram_id
      try {
        const existingUser = await userService.getUserByTelegramId(telegram_id);
        if (existingUser) {
          logger.info(`[TG REGISTERED] Пользователь с telegram_id=${telegram_id} уже существует, возвращаем существующего`);
        ...
        logger.info(`[TG REGISTERED] Пользователь с telegram_id=${telegram_id} не найден, создаем нового`);
      }
      
      // Генерируем уникальный реферальный код
      const ref_code = `ref_${telegram_id}_${Math.random().toString(36).substr(2, 8)}`;
      ...
      logger.info(`[TG REGISTERED] Пользователь создан: telegram_id=${telegram_id}, user_id=${createdUser.id}, ref_code=${ref_code}`);
      
      return sendSuccess(res, {
        user_id: createdUser.id,
        telegram_id: createdUser.telegram_id,
        username: createdUser.username,
        ref_code: createdUser.ref_code,
        created_at: createdUser.created_at,
        isNewUser: true
      }, 'Telegram пользователь успешно зарегистрирован');
      
    } catch (error) {
      logger.error('[UserController] [TG REGISTRATION] Ошибка при регистрации Telegram пользователя:', error);
      sendServerError(res, 'Ошибка при регистрации Telegram пользователя');
    }
  }
};