...
export const UserController = {
  ...
  async getUserByGuestId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Критические ошибки - логируем и возвращаем 500
      logger.error(`[UserController] ❌ Критическая ошибка при получении пользователя по guest_id: ${req.params.guest_id}`, error);
      next(error);
    }
  },
