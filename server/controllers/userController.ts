...
export const UserController = {
  ...
  async getUserByGuestId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.info(`[UserController] Запрос пользователя по guest_id: ${guestId}`);
      ...
      try {
        ...
        if (user) {
          ...
          // Пользователь не найден - это нормальная ситуация для новых пользователей
          logger.info(`[UserController] ℹ️ Пользователь с guest_id: ${guestId} не найден (новый пользователь)`);
          sendError(res, 'Пользователь с указанным guest_id не найден', 404);
        }
      } catch (serviceError) {
        // Логируем ошибку сервиса, но НЕ пропагируем её как 500
        logger.error(`[UserController] ❌ Ошибка сервиса при получении пользователя по guest_id: ${guestId}`, serviceError);

        // ВСЕ сервисные ошибки трактуем как "пользователь не найден"
        // Это предотвращает 500 ошибки и обеспечивает корректную работу клиента
        logger.warn(`[UserController] 🔄 Все сервисные ошибки трактуются как "пользователь не найден" для стабильности API`);
        sendError(res, 'Пользователь с указанным guest_id не найден', 404);
      }
    } catch (criticalError) {
      // Только критические ошибки уровня контроллера пропускаем в middleware
      logger.error(`[UserController] 💥 КРИТИЧЕСКАЯ ошибка в getUserByGuestId:`, criticalError);
      
      // Даже критические ошибки возвращаем как 404 для стабильности API
      sendError(res, 'Пользователь с указанным guest_id не найден', 404);
    }
  },
