-- Сначала архивируем старые таблицы (если они еще не архивированы)
ALTER TABLE IF EXISTS uni_farming_data RENAME TO _archived_uni_farming_data_2025_08_02;
ALTER TABLE IF EXISTS ton_farming_data RENAME TO _archived_ton_farming_data_2025_08_02;
...
SELECT 
  ...
  CASE 
    WHEN ton_boost_package_id IS NOT NULL THEN ton_boost_package_id::INTEGER
    ELSE ton_boost_package
  END as boost_package_id,
  created_at,
  COALESCE(ton_farming_last_update, created_at) as updated_at
FROM users
WHERE ton_wallet_address IS NOT NULL OR ton_farming_balance > 0 OR ton_boost_package IS NOT NULL OR ton_boost_package_id IS NOT NULL;