...
import { directBalanceCheck } from './directBalanceHandler';
import { z } from 'zod';
...
router.get('/balance', requireTelegramAuth, massOperationsRateLimit, walletController.getBalance.bind(walletController));
...
router.post('/ton-deposit', requireTelegramAuth, strictRateLimit, validateBody(tonDepositSchema), walletController.tonDeposit.bind(walletController)); // Пополнение через TON (УСИЛЕНА ЗАЩИТА)
