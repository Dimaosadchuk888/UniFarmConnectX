...
/**
 * Таблица уровней реферальной программы — DEBUG ВЕРСИЯ
...
const ReferralLevelsTable: React.FC = () => {
  // Для проверки кода
  console.log('[ReferralLevelsTableDebug] Компонент запущен!');
  ...
  const { data: referralsData, isLoading, error } = useQuery<ReferralsResponse>({
    ...
    queryFn: async () => {
      try {
        ...
        console.error('[ReferralLevelsTableDebug] Ошибка запроса:', error);
  ...
  const levels: ReferralLevel[] = React.useMemo(() => {
    ...
    try {
      ...
      console.log('[ReferralLevelsTableDebug] API данные:', { 
        referralCounts, 
        levelIncome,
      ...
      console.error('[ReferralLevelsTableDebug] Ошибка обработки данных:', error);
  ...
  return (
    <div className="bg-card rounded-xl p-4 shadow-lg relative overflow-hidden">
      {/* Маркер дебаг версии */}
      <div className="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-1 rounded z-50">DEBUG</div>
