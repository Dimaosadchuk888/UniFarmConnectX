...
async function main() {
  ...
  // Добавляем задержку перед проверкой результатов
  console.log('⏳ Ожидание применения изменений в API Telegram (5 секунд)...');
  await delay(5000);
  
  // Проверяем, что настройки обновились
  console.log('🔍 Проверка применения изменений...');
  const updatedMenuButton = await getCurrentMenuButton();
  
  if (updatedMenuButton.text === 'Открыть приложение') {
    console.log('✅ Проверка успешна: текст кнопки меню обновлен!');
  } else {
    console.log('⚠️ Предупреждение: текст кнопки меню не обновился. Повторяем запрос...');
    await updateMenuButton();
  }
